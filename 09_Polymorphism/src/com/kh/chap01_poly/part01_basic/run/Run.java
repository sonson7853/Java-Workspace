...
public class Run {
	...
	public static void main(String[] args) {
		...
		//c1 레퍼런스변수가 Parent에 있는 메소드에 접근시 자동으로 현변환된 채 진행됨.
		//즉 자식에서 부모클래스로 자동형변환(업캐스팅) -> 작은것에서 큰것으로 형변환 된것과 같은이치.
		
		// 3. 부모타입 레퍼런스 변수로 자식 객체를 다루는 경우(다형성 적용된 개념)
		System.out.println("부모타입 레퍼런스로 자식 객체를 다루는경우(다형성 적용된 경우)");
		Parent p2 = (Parent) new Child1();
		/*
		 * 양쪽에 자료형이 다른데도 불구하고 오류가 발생하지 않는다. 
		 * Parent형으로 자료형변환이 되고 있기 때문에 
		...
		//  단, 다형성이 적용되면 부모타입레퍼런스 변수로 모든 자식객체를 받아줄 수있다.
		...
		// 하나의 부모타입으로 모든 자식객체들을 받을 수 있음 => 관리, 코드수 감소, 생산성이 좋다.
...
//			}
		...
		arr[i].print();
		/*
		 *  * 동적바인딩 : 프로그램이 실행된기전에 자동으로 컴파일되면서 정적으로 현재 레퍼런스 변수의 메소드를 가
		 *  단, 참조하고있는 자식클래스에 해당 메소드가 오버라이딩 되어 있다면
