...
public class A_UncheckedException {
	 ...
	 * - NullPointerException : 레퍼런스가 아직 null임에도 불구하고 접근하려고 할때 발생하는 에러
	...
	public void method1() {
		...
		// 사용자에게 두개의 정수 값을 입력받아 나눗셈 연산 결과 출력
		System.out.print("첫 번째 정수 : ");
		int num1 = sc.nextInt();
		System.out.print("두 번째 정수 : ");
		int num2 = sc.nextInt();
		
		// 해결방법1. 조건문으로 처리(애초에 예외자체가 발생되지 않게끔 if문으로 조건검사하기) => 예외처리x
//		if(num2 == 0) {
...
//		System.out.println("프로그램 종료.");
		...
		try {
			...
			e.printStackTrace(); // 현재발생한 오류를 추적할수있는 로그정보제공메소드
		}
		System.out.println("프로그램 종료");		
	}
	...
	public void method2() {
		...
		try {
			int num = sc.nextInt(); // InputMismatchException : 정수 이외의값을 입력해서 에러
			System.out.println("나누셈 연산 결과 : " + (10/num));//예외 발생할수 있음			
		} catch(ArithmeticException e) {
			System.out.println("0으로 나눌 수 없습니다.");
		} catch(InputMismatchException e) {
			System.out.println("정수를 제대로 입력해주세요.");
		} //다중 catch 블럭으로 사용가능 : 에외가 여러개 발생할 가능성이 있는 경우
...
//			int [] arr = new int[Math.abs(size)]; // 방법2. 항상 양의 정수값이 들어갈수있도록 abs메소드 사용
//		}
...
//		} // 다중 catch블럭이 줄어들었음. 정확히 어떤 에러인지 알 수 없음.
	 ...
	 *  - 예외처리 구무으로도 해결 가능 => 예외가 발생했을때를 대비해서 그때 실행할 내용을 정의해두는것.
	 *  
	 * 예측이 가능한 상황 => 조건문으로 해결(권장하는방법)
	 * 예측이 불가능한 상황 => 예외처리 구문으로 해결 
	 * 
	 * RuntimeException 계열은 충분히 에측 가능한 상황이기때문에 조건문으로 해결하는것을 권장
	 * 
	 * UnchecekdException -> 필수로 체크하지는 않아도 되는 예외라는 뜻
	 */
}
