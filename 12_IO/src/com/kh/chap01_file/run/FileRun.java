...
public class FileRun {
	...
	public static void main(String[] args) {
		...
		try {
			...
			String dir = "C:\\bbb";
			File bbbFolder = new File(dir); //폴더생성
			if(!bbbFolder.isDirectory()) {
				bbbFolder.mkdir();
			}
			File file3 = new File(dir+"\\test.txt"); //생성한 폴더안에 txt파일생성
			file3.createNewFile();
		 ...
		 * 입력장치(키보드,마우스)로 입력받는 과정을 진행하고자 한다면 반드시 외부매체와의 통로를 만들어야한다.
		 * 통로 == 스트림
		 * 
		 * 스트림의 특징 :
		 *  - 단방향 : 입력이면 입력, 출력이면 출력
		 * 			 입력, 출력용 스트림이 따로 존재
		 * 			 즉, 동시에 입출력하고자한다면 하나의 스트림으로는 불가능함.
		 * 
		 ...
		 * 스트림의 구분
		 * - 통로의 사이즈 : 
		 * 	 바이트 스트림 : 1BYTE짜리가 이동할 수 있을정도의 사이즈(좁은통로) 
		 * 						입력 =(XXXInputStream) 출력= (XXXOutputStream)
		 * 
		 * 	 문자 스트림  : 2BYTE짜리가 이동할 수 있을정도의 사이즈(넓은통로)
		 *  				    입력 =(XXXRedaer) 출력 = (XXXWriter)
		 *  
		 * - 외부매체와의 직접적인 연결여부 :
