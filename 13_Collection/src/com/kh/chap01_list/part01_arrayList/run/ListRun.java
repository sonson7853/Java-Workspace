...
public class ListRun {
	...
	public static void main(String[] args) {
		 ...
		 * 3. 기본적으로 여러 타입의 데이터들을 저장 할 수 있음(단, 제네릭 설정을 통해서 한 타입의 데이터들만 들어올 수도있음)
		 *  
		 * 방대한 데이터들을 단지 담아만 두고 조회 할 목적 => 배열
		 * 방대한 데이터들이 번번하게 추가, 삭제, 수정 될 것 같다 => 컬렉션
		 * 
		 * 컬렉션의 3가지 분류
		 * 
		 * List계열 => 담고자 하는 값(value)만 저장/ 값 저장 시 순서유지됨(index의 개념이 있음)/ 중복값 허용.
		 * 			ex) ArrayList, Vector, LinkedList => ArrayList가 제일많이 쓰임
		 * Set 계열 => 담고자 하는 값(value)만 저장/ 값 저장 시 순서 유지안됨(index개념이 없음)/ 중복값 허용x
		 * 			ex) HashSet, TreeSet
		 * Map 계열 => 키(key) + 담고자하는 값(value)세트로 저장 / 값 저장시 순서 유지x(index 개념 없음)
		 *           / key 중복 허용 x
		 ...
		 * 2. 컬렉션에 저장된 객체를 꺼내서 사용할 때 매번 형 변환 하는 절차를 없애기 위해서.
		 */
		...
		// 중간에 값 추가 시 알아서 기존의 값들을 뒤로 땡져주는 작업이 내부적으로 진행됨.
		System.out.println(list);
		...
		// for each문 : 향상된 반복문
		// for(값을 담아 줄 변수 생성 : 순차적으로 접근할 배열명, 컬렉션)
		for(Music o : list) {
