...
//		hs1.add(50)
		...
		// 값이 추가될때 equals(), hsahCode()로 비교후 둘다 결과가 true면 동일객체로 판단.
		
		//HashSet에 객체를 담을 때 내부적으로 equals()로 비교 + hashCode()값이 일치하는지도 비교
		//eqauals() 한 결과가 true이고 hashCode()값이 일치하면 = > 동일객체로 판단 (중복저장x)
		...
		//set => 무작위로 객체가 저장되어있음. (index 개념이 없음) => 반목문 활용이 불가
		//1. for문 사용가능( 단, for each문(향상된 방복문)만 사용가능 =>index로 접근이 불가하므로 
		for(Student s : hs2) {
		...
		//hs2에 담겨있는 객체들을 itegrator(반복자)에 담는 과정
		Iterator<Student> it = hs2.iterator();
		
		while(it.hasNext()) {//it에 next로 빼올값이 있는가 ? 라고 물어보는 메서드
		it.next();
		
		}//String의 StringTokenizer비슷한 개념
		//.terator(): lsit계열과 set계열에서만사용가능함
